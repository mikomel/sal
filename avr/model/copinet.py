...
class CoPINet(nn.Module):
    def __init__(
        ...
        self.res2 = ResBlock(
            ...
            256,
            stride=2,
            downsample=nn.Sequential(conv1x1(128, 256, stride=2), nn.BatchNorm2d(256)),
        )

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

        self.mlp = nn.Sequential(nn.Linear(256, embedding_size), nn.ReLU(inplace=True))
    ...
    def forward(
        ...
        row_features = self.relu(
            self.inf_bn_row(
                self.inf_conv_row(torch.cat((row1_features, row2_features), dim=0))
            )
        )
        ...
        col_features = self.relu(
            self.inf_bn_col(
                self.inf_conv_col(torch.cat((col1_features, col2_features), dim=0))
            )
        )
        ...
        choices_features = input_features[:, 8:, :, :, :].unsqueeze(
            2
        )  # N, 8, 64, 20, 20 -> N, 8, 1, 64, 20, 20

        row1_features = torch.sum(
            input_features[:, 0:3, :, :, :], dim=1
        )  # N, 64, 20, 20
        row2_features = torch.sum(
            input_features[:, 3:6, :, :, :], dim=1
        )  # N, 64, 20, 20
        row3_pre = (
            input_features[:, 6:8, :, :, :]
            .unsqueeze(1)
            .expand(N, num_answers, 2, 64, 20, 20)
        )  # N, 2, 64, 20, 20 -> N, 1, 2, 64, 20, 20 -> N, 8, 2, 64, 20, 20
        row3_features = torch.sum(
            torch.cat((row3_pre, choices_features), dim=2), dim=2
        ).view(
            -1, 64, 20, 20
        ...
        row_features = self.relu(
            self.bn_row(
                self.conv_row(
                    torch.cat((row1_features, row2_features, row3_features), dim=0)
                )
            )
        )
        ...
        row1 = (
            ...
            .unsqueeze(1)
            .unsqueeze(1)
            .expand(N, num_answers, 1, 64, 20, 20)
        )
        row2 = (
            row_features[N : 2 * N, :, :, :]
            .unsqueeze(1)
        ...
        row3 = (
            row_features[2 * N :, :, :, :]
            .view(-1, num_answers, 64, 20, 20)
            .unsqueeze(2)
        )
        final_row_features = torch.sum(torch.cat((row1, row2, row3), dim=2), dim=2)

        col1_features = torch.sum(
            input_features[:, 0:9:3, :, :, :], dim=1
        )  # N, 64, 20, 20
        col2_features = torch.sum(
            input_features[:, 1:9:3, :, :, :], dim=1
        )  # N, 64, 20, 20
        col3_pre = (
            input_features[:, 2:8:3, :, :, :]
            .unsqueeze(1)
            .expand(N, num_answers, 2, 64, 20, 20)
        )  # N, 2, 64, 20, 20 -> N, 1, 2, 64, 20, 20 -> N, 8, 2, 64, 20, 20
        col3_features = torch.sum(
            torch.cat((col3_pre, choices_features), dim=2), dim=2
        ).view(
            -1, 64, 20, 20
        )  # N, 8, 3, 64, 20, 20 -> N, 8, 64, 20, 20 -> N * 8, 64, 20, 20
        col_features = self.relu(
            self.bn_col(
                self.conv_col(
                    torch.cat((col1_features, col2_features, col3_features), dim=0)
                )
            )
        )
